...
CREATE INDEX IF NOT EXISTS "Person_status_deleted_customerNo_idx"
  ON "Person" ("status", "deletedAt", "customerNo");

CREATE INDEX IF NOT EXISTS "Person_status_deleted_created_idx"
...
CREATE INDEX IF NOT EXISTS "Person_customerNo_trgm_idx"
  ON "Person" USING gin ("customerNo" gin_trgm_ops);

CREATE INDEX IF NOT EXISTS "Person_externalId_trgm_idx"
  ON "Person" USING gin ("externalId" gin_trgm_ops);

CREATE INDEX IF NOT EXISTS "FT_person_deleted_txAt_idx"
  ON "FinancialTransaction" ("personId", "deletedAt", "transactionAt");

CREATE INDEX IF NOT EXISTS "FT_deleted_status_txAt_idx"
  ON "FinancialTransaction" ("deletedAt", "status", "transactionAt");

CREATE INDEX IF NOT EXISTS "FT_deleted_kind_txAt_idx"
  ON "FinancialTransaction" ("deletedAt", "operationKind", "transactionAt");

CREATE INDEX IF NOT EXISTS "CWS_person_deleted_occ_idx"
  ON "CustomerWalletSettlement" ("personId", "deletedAt", "occurredAt");

CREATE INDEX IF NOT EXISTS "CWS_person_currency_method_type_deleted_occ_idx"
  ON "CustomerWalletSettlement" ("personId", "currencyId", "paymentMethod", "accountType", "deletedAt", "occurredAt");

CREATE INDEX IF NOT EXISTS "RCB_person_receivedAt_idx"
  ON "ReceivedCardBatch" ("personId", "receivedAt");

CREATE INDEX IF NOT EXISTS "RCC_batch_deleted_sequence_idx"
  ON "ReceivedCustomerCard" ("batchId", "deletedAt", "sequence");

CREATE INDEX IF NOT EXISTS "RCC_status_deleted_updated_idx"
  ON "ReceivedCustomerCard" ("status", "deletedAt", "updatedAt");

CREATE INDEX IF NOT EXISTS "RCC_settlementCurrency_deleted_status_idx"
  ON "ReceivedCustomerCard" ("settlementCurrencyId", "deletedAt", "status");

CREATE INDEX IF NOT EXISTS "RCSL_card_created_idx"
  ON "ReceivedCardStageLog" ("cardId", "createdAt");

CREATE INDEX IF NOT EXISTS "RCO_card_deleted_occurred_idx"
  ON "ReceivedCardOperation" ("cardId", "deletedAt", "occurredAt");

CREATE INDEX IF NOT EXISTS "RCO_type_deleted_occurred_idx"
  ON "ReceivedCardOperation" ("operationType", "deletedAt", "occurredAt");

CREATE INDEX IF NOT EXISTS "CCD_person_currency_deleted_occurred_idx"
